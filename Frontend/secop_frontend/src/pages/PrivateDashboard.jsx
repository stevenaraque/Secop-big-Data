...
export default function PrivateDashboard({ token }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#fcfcfc] dark:bg-zinc-950 text-zinc-900 antialiased">
        ...
        <section className="lg:col-span-1 space-y-6">
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
            ...
            <form onSubmit={handleCrear} className="mt-4 space-y-3">
              <label className="block text-xs font-medium">Departamento
                <input value={form.departamento_objetivo} onChange={(e) => setForm({ ...form, departamento_objetivo: e.target.value })} placeholder="Boyaca" className="mt-1 w-full h-9 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 text-sm" />
              </label>
              <label className="block text-xs font-medium">Palabras clave *
                <input value={form.palabras_clave} onChange={(e) => setForm({ ...form, palabras_clave: e.target.value })} placeholder="pavimento" required className="mt-1 w-full h-9 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 text-sm" />
              </label>
              <div className="grid grid-cols-2 gap-2">
                <label className="block text-xs font-medium">Min
                  <input type="number" value={form.rango_cuantia_min} onChange={(e) => setForm({ ...form, rango_cuantia_min: e.target.value })} placeholder="5000000" className="mt-1 w-full h-9 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 text-sm font-mono" />
                </label>
                <label className="block text-xs font-medium">Max
                  <input type="number" value={form.rango_cuantia_max} onChange={(e) => setForm({ ...form, rango_cuantia_max: e.target.value })} placeholder="30000000" className="mt-1 w-full h-9 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 text-sm font-mono" />
                </label>
              </div>
              <label className="block text-xs font-medium">Filtros 85 cols (JSON)
                <input value={form.filtros_extras} onChange={(e) => setForm({ ...form, filtros_extras: e.target.value })} placeholder='{"ciudad":"Sogamoso"}' className="mt-1 w-full h-9 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 text-xs font-mono" />
              </label>
              <button disabled={crearRadar.isPending} className="w-full h-9 rounded-lg bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 text-sm font-medium hover:bg-zinc-800 disabled:opacity-50">
                {crearRadar.isPending ? "Creando..." : "Crear Radar"}
              </button>
              {msg && <p className="text-xs text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-900 rounded-lg px-3 py-2">{msg}</p>}
            </form>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
            ...
            <div className="mt-3 space-y-2 max-h-[320px] overflow-auto pr-1">
              {radares.map((r) => (
                <div key={r.id} className="rounded-xl border border-zinc-200 dark:border-zinc-700 p-3 flex items-start justify-between gap-2">
                  ...
                  <span className={`text-[11px] px-2 py-1 rounded-full border ${r.activo ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:border-emerald-900" : "bg-zinc-50 dark:bg-zinc-800/60 text-zinc-600 dark:text-zinc-400 border-zinc-200 dark:border-zinc-700"}`}>{r.activo ? "Activo" : "Pausado"}</span>
                </div>
              ))}
        ...
        <section className="lg:col-span-2">
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-[11px] px-2 py-1 rounded-full bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 font-mono">{count} total</span>
            </div>
              ...
              {(Array.isArray(oportunidades) ? oportunidades : []).map((op) => (
                <div key={op.id} className="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-zinc-500 dark:text-zinc-400">Radar: {op.radar_palabras} · {new Date(op.creado_en).toLocaleString("es-CO")}</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    {op.estado === "Nueva" && (
                      <>
                        ...
                        <button onClick={() => cambiarEstado.mutate({ id: op.id, estado: "Postulado" })} className="h-7 px-3 rounded-full border border-zinc-200 dark:border-zinc-700 text-xs hover:bg-zinc-50 dark:hover:bg-zinc-800">Postular</button>
                      </>
                    )}
                    {op.estado === "Guardada" && (
