    ...
    <div className="min-h-[100dvh] bg-[#fcfcfc] dark:bg-zinc-950 relative overflow-hidden flex flex-col">
      ...
      <main id="contenido" className="flex-1 w-full max-w-[1400px] mx-auto px-4 lg:px-8 pb-10 lg:pb-12 grid grid-cols-1 lg:grid-cols-[1.18fr_0.92fr] gap-8 lg:gap-10 items-start lg:items-center">
        <motion.div variants={stagger} initial="hidden" animate="show" className="pt-2 lg:pt-0 lg:pl-[1.5vw] order-2 lg:order-1">
          <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-2"><LivePulse /><span className="inline-flex items-center gap-1.5 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-3 py-1 text-[11px] font-medium"><Lightning size={12} weight="fill" /> 1 solo uso · atómico</span></motion.div>
          <motion.h1 variants={fadeUp} className="mt-6 text-4xl md:text-[52px] lg:text-[56px] font-semibold tracking-tighter leading-none text-zinc-900 dark:text-white" style={{ fontFamily: "Geist, Satoshi, ui-sans-serif" }}>Nueva<br /><span className="text-zinc-500 dark:text-zinc-400">contraseña segura.</span></motion.h1>
          <motion.p variants={fadeUp} className="mt-4 text-base leading-relaxed text-zinc-600 dark:text-zinc-400 max-w-[58ch]">Pega el token del enlace y define clave <span className="font-medium text-zinc-900 dark:text-white">8+ may/min/número</span> con PBKDF2. El token se quema tras usar — reintento da <span className="font-mono text-xs">Enlace ya utilizado</span>.</motion.p>
          <motion.div variants={fadeUp} className="mt-8 grid grid-cols-2 gap-4 max-w-[560px]">
            {[
              { icon: Key, k: "SHA-256", label: "hash", sub: "raw nunca en BD" },
              { icon: ClockClockwise, k: "30 min", label: "expira", sub: "esta_expirado()" },
              { icon: ShieldCheck, k: "PBKDF2", label: "set_password", sub: "may/min/número" },
              { icon: LockKey, k: "atomic", label: "select_for_update", sub: "anti doble-uso" },
            ].map((c) => (
          ...
          <div className="rounded-[2.5rem] bg-white/85 dark:bg-zinc-900/80 backdrop-blur-xl border border-white/10 dark:border-zinc-800 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.1)] overflow-hidden">
            <div className="p-8 md:p-9">
              <div className="flex items-start justify-between gap-4">
                <div><h2 className="text-[18px] font-semibold tracking-tight text-zinc-900 dark:text-white" style={{ fontFamily: "Geist, Satoshi, ui-sans-serif" }}>Restablecer contraseña</h2><p className="text-sm leading-relaxed text-zinc-600 dark:text-zinc-400 mt-1">Enlace 30 min, un solo uso.</p></div>
              ...
              <form onSubmit={handleSubmit} className="mt-7 space-y-4" noValidate>
                <UiverseInput id="restablecer-token" label="Token" value={token} onChange={(e) => setToken(e.target.value)} placeholder="token del enlace" required autoComplete="off" delay={0} />
                ...
                <MagneticButton type="submit" disabled={estado === "loading"} className="w-full">
                  {estado === "loading" ? <><span className="size-3.5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Guardando…</> : <>Restablecer <ArrowRight size={16} weight="bold" /></>}
                </MagneticButton>
              </form>
      ...
      <footer className="w-full max-w-[1400px] mx-auto px-4 lg:px-8 py-6 flex items-center justify-between gap-3 text-[11px] text-zinc-500 border-t border-zinc-200/50 dark:border-zinc-800 mt-auto"><span>© 2026 SECOP Insight · Restablecer</span><span>30 min · un solo uso</span></footer>
    </div>
  );
}
