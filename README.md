...
- **RF-04 — Login — COMPLETADO y VALIDADO (07/09/2026):**
  ...
  - Frontend (15/09): `Frontend/secop_frontend/src/pages/Login.jsx` form correo+contraseña → `POST /api/auth/login/` guarda `access+refresh` y va a `/`; `App.jsx` ruta `/login` + sin token redirige a `/login`; `Dashboard.jsx` botón Salir invalida refresh + limpia local. Verificado `npm run build` OK
...
- **Sesión 5 / Buffer (Semana 5) — PENDIENTE 15/09:** pushear login frontend (`Login.jsx` en disco) + suite mínima de pruebas (hoy cero: sin pytest/Jest) + ramas `feature/*` desde ahora + video 3 min + pitch 45s + tag `v1.0-sprint4` + Swagger `/api/docs/` + Notion al día + reflexión 3.1. Deploy local-first luego Render/Vercel. Ver `CONTEXT.md:9` (revisión Guía 4 con estado de evidencias).
