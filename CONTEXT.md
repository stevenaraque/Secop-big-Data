...
- **API RF-08 resumen DONE completo (07/09/2026):** `contratos/views.py:45` `VistaResumenOptimizado` `IsAuthenticated` `aggregate Count/Sum/Avg` con `idx_contrato_depto/modalidad/fecha` + `VistaResumenNaive` `sum()` Python, `contratos/urls.py:1` `optimized/resumen/` + `naive/resumen/` → `GET api/optimized/resumen/?depto=Boyacá&anio=2025&modalidad=Contratación directa` verificado `Invoke-RestMethod` `Bearer` `optimizado True total 0` + `naive False total 7` + `todos total 10 suma 224M` + `filtro {depto,anio,modalidad}` OK. RF-08 3 filtros `depto/anio/modalidad` validado `fecha_firma__year` + `departamento` + `modalidad`. Patrón 50KB vs 100MB validado.
